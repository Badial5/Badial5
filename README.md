<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:09090b,50:1e293b,100:2563eb&height=180&section=header&text=Calvin%20Williams&fontSize=42&fontColor=ffffff&animation=fadeIn" width="100%" />

  <a href="https://git.io/typing-svg">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&pause=1000&color=38BDF8&center=true&vCenter=true&width=620&lines=Frontend+%26+Full-Stack+Software+Engineer;React+%7C+Next.js+%7C+TypeScript+%7C+React+Native;Modern+Web+Apps+%7C+Offline-First+Mobile;Available+for+International+Remote+Roles" alt="Typing SVG" />
  </a>

  <p align="center">
    <a href="https://mail.google.com/mail/?view=cm&fs=1&to=calvinwilliamsbusiness@gmail.com&su=Remote+Engineering+Opportunity">
      <img src="https://img.shields.io/badge/Status-Available_for_Remote_Roles-22c55e?style=for-the-badge&logo=statuspage&logoColor=white" alt="Remote Status" />
    </a>
    <a href="https://calvin-williams.vercel.app/">
      <img src="https://img.shields.io/badge/Portfolio-calvin--williams.vercel.app-2563eb?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" />
    </a>
    <a href="https://wa.me/233557279987?text=Hi%20Calvin,%20I%20came%20across%20your%20GitHub%20profile%20and%20would%20like%20to%20connect">
      <img src="https://img.shields.io/badge/WhatsApp-Chat_Directly-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" alt="WhatsApp" />
    </a>
  </p>
</div>

---

### 👨🏾‍💻 Executive Summary

> **Frontend & Full-Stack Engineer** with 3+ years of experience engineering high-performance web applications, responsive commerce platforms, and offline-first mobile tools.

- 🌍 **Location & Timezone:** Ghana 🇬🇭 (GMT / UTC+0) — *Seamless working overlap with US, UK, European, and Pan-African teams.*
- ⚡ **Engineering Core:** Strict TypeScript safety, performant state caching (TanStack Query), modular UI architecture, and zero-compromise UX.
- 📱 **Mobile & Hardware:** React Native (Expo) mobile apps and browser hardware integrations (Web Serial API & ESC/POS thermal printing).
- 💼 **Open to:** Full-time Remote roles, high-impact contract engineering, and frontend consulting.

---

### 🛠️ Tech Stack & Tooling

<table>
  <tr>
    <td width="22%"><b>Frontend & Mobile</b></td>
    <td>
      <img src="https://img.shields.io/badge/React_19-20232A?style=flat-square&logo=react&logoColor=61DAFB" />
      <img src="https://img.shields.io/badge/Next.js_15-000000?style=flat-square&logo=nextdotjs&logoColor=white" />
      <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" />
      <img src="https://img.shields.io/badge/React_Native-20232A?style=flat-square&logo=react&logoColor=61DAFB" />
      <img src="https://img.shields.io/badge/Expo-000020?style=flat-square&logo=expo&logoColor=white" />
      <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white" />
      <img src="https://img.shields.io/badge/Shadcn_UI-000000?style=flat-square&logo=shadcnui&logoColor=white" />
    </td>
  </tr>
  <tr>
    <td><b>State & Architecture</b></td>
    <td>
      <img src="https://img.shields.io/badge/TanStack_Query-FF4154?style=flat-square&logo=reactquery&logoColor=white" />
      <img src="https://img.shields.io/badge/Zustand-443E38?style=flat-square&logo=redux&logoColor=white" />
      <img src="https://img.shields.io/badge/Zod_Validation-3E67B1?style=flat-square&logo=zod&logoColor=white" />
      <img src="https://img.shields.io/badge/Web_Serial_API-010101?style=flat-square&logo=googlechrome&logoColor=white" />
      <img src="https://img.shields.io/badge/Offline--First-10B981?style=flat-square&logo=pwa&logoColor=white" />
    </td>
  </tr>
  <tr>
    <td><b>Backend & APIs</b></td>
    <td>
      <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white" />
      <img src="https://img.shields.io/badge/Express.js-000000?style=flat-square&logo=express&logoColor=white" />
      <img src="https://img.shields.io/badge/REST_APIs-005571?style=flat-square&logo=fastapi&logoColor=white" />
      <img src="https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white" />
      <img src="https://img.shields.io/badge/JWT_Auth-000000?style=flat-square&logo=jsonwebtokens&logoColor=white" />
    </td>
  </tr>
  <tr>
    <td><b>DevOps & Quality</b></td>
    <td>
      <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" />
      <img src="https://img.shields.io/badge/Vite-646CFF?style=flat-square&logo=vite&logoColor=white" />
      <img src="https://img.shields.io/badge/Vercel-000000?style=flat-square&logo=vercel&logoColor=white" />
      <img src="https://img.shields.io/badge/Postman-FF6C37?style=flat-square&logo=postman&logoColor=white" />
      <img src="https://img.shields.io/badge/Lighthouse_95+-00C49F?style=flat-square&logo=lighthouse&logoColor=white" />
    </td>
  </tr>
</table>

---

### 💼 Featured Projects & Live Demos

<table>
  <tr>
    <td width="50%" valign="top">
      <h4>⚖️ Legal Diary (Mobile App)</h4>
      <p><b>Offline-First Judicial Case Management</b></p>
      <ul>
        <li>React Native (Expo SDK 57) mobile app with <b>on-device OCR text recognition</b> (Google ML Kit).</li>
        <li>Automated procedural deadline engine per civil procedure rules (C.I. 47) with cascading reminders.</li>
        <li>Hardware-backed encrypted docket vault running 100% offline-first.</li>
      </ul>
      <p>
        <a href="https://mail.google.com/mail/?view=cm&fs=1&to=calvinwilliamsbusiness@gmail.com&su=Live+Demo+Request+-+Legal+Diary+Mobile+App&body=Hi+Calvin,+I+would+like+to+request+a+live+demo/walkthrough+of+the+Legal+Diary+Mobile+App.">
          <img src="https://img.shields.io/badge/📱_Request_Live_Demo-2563EB?style=for-the-badge&logoColor=white" alt="Request Live Demo" />
        </a>
      </p>
      <p>
        <code>React Native</code> <code>Expo</code> <code>NativeWind</code> <code>Zustand</code> <code>ML Kit</code>
      </p>
    </td>
    <td width="50%" valign="top">
      <h4>💰 Microfinance & Susu Savings</h4>
      <p><b>Grassroots Digital Lending Platform</b></p>
      <ul>
        <li>Full-featured micro-lending web app with daily Susu contribution logging and balance reconciliation.</li>
        <li>Dynamic loan amortization calculator with sub-50ms optimistic state updates via TanStack Query.</li>
        <li>Mobile-first responsive architecture scoring 98/100 on Lighthouse.</li>
      </ul>
      <p>
        <a href="https://microfinance-lac.vercel.app/">
          <img src="https://img.shields.io/badge/🔗_Live_App-22C55E?style=for-the-badge&logoColor=white" alt="Live App" />
        </a>
        <a href="https://mail.google.com/mail/?view=cm&fs=1&to=calvinwilliamsbusiness@gmail.com&su=Live+Demo+Request+-+Microfinance+Platform&body=Hi+Calvin,+I+would+like+to+request+a+live+demo+of+the+Microfinance+Platform.">
          <img src="https://img.shields.io/badge/📱_Request_Demo-2563EB?style=for-the-badge&logoColor=white" alt="Request Demo" />
        </a>
      </p>
      <p>
        <code>React 19</code> <code>TypeScript</code> <code>TanStack Query</code> <code>Tailwind CSS</code>
      </p>
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h4>🧾 NovaTrade Smart Receipt & POS Studio</h4>
      <p><b>Hardware POS & Receipt Generation Engine</b></p>
      <ul>
        <li>Direct ESC/POS thermal printer hardware integration using the native <b>Web Serial API</b> without third-party print drivers.</li>
        <li>Live WYSIWYG split-screen preview supporting A4, 80mm, and 58mm roll layouts.</li>
        <li>Dynamic currency to words conversion, PDF export, and full offline PWA capability.</li>
      </ul>
      <p>
        <a href="https://mail.google.com/mail/?view=cm&fs=1&to=calvinwilliamsbusiness@gmail.com&su=Live+Demo+Request+-+NovaTrade+POS+Studio&body=Hi+Calvin,+I+would+like+to+request+a+live+demo+of+the+NovaTrade+POS+Studio.">
          <img src="https://img.shields.io/badge/📱_Request_Live_Demo-2563EB?style=for-the-badge&logoColor=white" alt="Request Live Demo" />
        </a>
      </p>
      <p>
        <code>React</code> <code>TypeScript</code> <code>Tailwind CSS</code> <code>Web Serial API</code> <code>PWA</code>
      </p>
    </td>
    <td width="50%" valign="top">
      <h4>🌐 RanstadPlus Global Trade Platform</h4>
      <p><b>Commercial Agricultural Export Portal</b></p>
      <ul>
        <li>Modern trade catalog engineered with Next.js App Router for international commodity buyers.</li>
        <li>Automated product-tagged inquiry dispatcher using client-side EmailJS integration.</li>
        <li>Optimized static page generation with zero server overhead.</li>
      </ul>
      <p>
        <a href="https://mail.google.com/mail/?view=cm&fs=1&to=calvinwilliamsbusiness@gmail.com&su=Live+Demo+Request+-+RanstadPlus+Trade+Platform&body=Hi+Calvin,+I+would+like+to+request+a+live+demo+of+the+RanstadPlus+Platform.">
          <img src="https://img.shields.io/badge/📱_Request_Live_Demo-2563EB?style=for-the-badge&logoColor=white" alt="Request Live Demo" />
        </a>
      </p>
      <p>
        <code>Next.js</code> <code>TypeScript</code> <code>EmailJS</code> <code>Tailwind CSS</code>
      </p>
    </td>
  </tr>
</table>

---

### 📊 GitHub Activity & Real-Time Stats

<div align="center">
  <img src="https://streak-stats.demolab.com/?user=Badial5&theme=tokyonight&hide_border=true&card_width=480" alt="Calvin's GitHub Streak" />
</div>

<br />

<div align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Badial5&theme=tokyonight" alt="GitHub Profile Summary" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=Badial5&theme=tokyonight" alt="Top Languages" />
</div>

---

### 🤝 Let's Connect

I am actively open to **Full-Time Remote Roles**, **Contract Roles**, and **Consulting Opportunities** worldwide.

<p align="left">
  <a href="https://mail.google.com/mail/?view=cm&fs=1&to=calvinwilliamsbusiness@gmail.com&su=Let's+Connect+-+Remote+Opportunity">
    <img src="https://img.shields.io/badge/Gmail-calvinwilliamsbusiness%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
  <a href="https://calvin-williams.vercel.app/">
    <img src="https://img.shields.io/badge/Portfolio-calvin--williams.vercel.app-2563eb?style=for-the-badge&logo=vercel&logoColor=white" />
  </a>
  <a href="https://wa.me/233557279987?text=Hi%20Calvin,%20I%20would%20like%20to%20discuss%20a%20remote%20engineering%20opportunity">
    <img src="https://img.shields.io/badge/WhatsApp-Chat_Directly-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" />
  </a>
  <a href="https://github.com/Badial5">
    <img src="https://img.shields.io/badge/GitHub-Badial5-181717?style=for-the-badge&logo=github&logoColor=white" />
  </a>
</p>

<div align="center">
  <sub>⚡ Engineered for performance, reliability, and clean code. Built by Calvin Williams.</sub>
</div>
